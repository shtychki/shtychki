...
//						if($propCode == 'PROPERTY_COLOR_REF2')
						...
						} elseif (strpos($propCode, 'PROPERTY_cvet_') !== false){
							$prItems[$propName][$sort] = array(
								'NAME' 	=> $res['PREVIEW_PICTURE'],
								'LINK' 	=> $res['DETAIL_PAGE_URL'],
								'VALUE' => trim($val),
...
//							if ($tmp_color == '') {
							if ($propCode == 'PROPERTY_DLINA_KABELYA_M') {
								$res['PREVIEW_PICTURE'] = '';
								$prItems[$propName][$res['ID']] = array(
									'NAME' 	=> $res['PREVIEW_PICTURE'],
									'LINK' 	=> $res['DETAIL_PAGE_URL'],
									'VALUE' => $res['PROPERTY_DLINA_KABELYA_M_VALUE'],
									'ID'	=> $res['ID'],
									'SORT'	=> $sort,
									'CODE'	=> $arPropsID['XML_ID'],
									'TYPE'	=> 'number'
							...
							$arResult['test2'][] = $tmp_color.' - '.$res['DETAIL_PAGE_URL'];
						}
						$i++;
					}
					$arResult['test'][] = $res;
	...
	if (is_object($cp))
	{
		$cp->arResult['GENERAL_ID'] = $arResult['PROPERTIES']['ID']['VALUE'];
		if (isset($arResult['PROPERTIES']['BREADCRUMB_FILTER']['VALUE'])) {
			$cp->arResult['BREADCRUMB_FILTER_VALUE'] = $arResult['PROPERTIES']['BREADCRUMB_FILTER']['VALUE'];
			$cp->arResult['BREADCRUMB_FILTER_DESC'] = $arResult['PROPERTIES']['BREADCRUMB_FILTER']['DESCRIPTION'];
		}
		$cp->SetResultCacheKeys(array('GENERAL_ID','BREADCRUMB_FILTER_VALUE','BREADCRUMB_FILTER_DESC'));

		$arResult['GENERAL_ID'] = $cp->arResult['GENERAL_ID'];

		$count_res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['PROPERTIES']['ID']['VALUE'], array(), array('CODE'=>'EXTENDED_REVIEWS_COUNT'));
		if($count = $count_res->Fetch()) {
			$arResult['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE'] = IntVal($count["VALUE"]);
		}

		$raiting_res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $arResult['PROPERTIES']['ID']['VALUE'], array(), array('CODE'=>'EXTENDED_REVIEWS_RAITING'));
		if($raiting = $raiting_res->Fetch()) {
			$arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE'] = IntVal($raiting["VALUE"]);
		}
		if (isset($arResult['PROPERTIES']['BREADCRUMB_FILTER']['VALUE'])) {
			$arResult['BREADCRUMB_FILTER_VALUE'] = $cp->arResult['BREADCRUMB_FILTER_VALUE'];
			$arResult['BREADCRUMB_FILTER_DESC'] = $cp->arResult['BREADCRUMB_FILTER_DESC'];
		}
	}
}
?>