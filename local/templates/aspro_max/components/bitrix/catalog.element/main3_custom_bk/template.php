...
//echo 'ok';
//echo '<pre>';
//print_r($arResult['PROPERTIES']['EXTENDED_REVIEWS_RAITING']);
//echo '</pre>';
?>
...
<?if($arResult["OFFERS"] && $arParams["TYPE_SKU"]=="N"):?>
	...
	<?if($arResult["OFFERS"] && $arParams["TYPE_SKU"] !== "TYPE_1"):?>
		<?$this->SetViewTarget('PRODUCT_OFFERS_INFO');?>
			<div class="list-offers ajax_load  546456">
							...
							<?if(!$showCustomOffer || empty($arResult['OFFERS_PROP'])):?>
								<?if($countPhoto  > 1):?>
									<div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?=($bVertical ? ' product-detail-gallery__slider--vertical' : '');?><?=($countPhoto > 3 ? ' m-photo' : '');?>" data-size="<?=$countPhoto;?>" data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider.big", "dots": false, "autoWidth": true, "margin": 10<?//if($bVertical):?>, "mouseDrag": false, "pullDrag": false<?//endif;?><?if($bMagnifier):?>, "magnifier": true<?endif;?>}' style="max-width:<?=ceil((($countPhoto <= 4 ? $countPhoto : 4) * 70) - 10)?>px;">
										<?if($arResult["MORE_PHOTO"]):?>
											<?foreach($arResult["MORE_PHOTO"] as $i => $arImage):?>
												<div id="more-photo-<?=$i?>" class="product-detail-gallery__item text-center  product-detail-gallery__item--thmb" data-big="<?=$arImage["BIG"]["src"];?>">
													<?if(!$isEmpty){?>
				...
				<div class="flexbox flexbox--row flex-wrap align-items-normal <?=!$bShowPropsBlock ? 'justify-center' : ''?>">
					<?if($bShowPropsBlock):?>
						<div class="product-chars flex-50">
							...
							<?endif;?>
							<?include_once('page_blocks/element_detail_linking.php');?>
						</div>
					<?endif;?>
...
<script>
BX.ready(function(){
	var buy = <?=CUtil::PhpToJSObject($buy_html, false, true);?>;
	var payment = <?=CUtil::PhpToJSObject($payment_html, false, true);?>;
	var delivery = <?=CUtil::PhpToJSObject($delivery_html, false, true);?>;
	var stores = <?=CUtil::PhpToJSObject($stores_html, false, true);?>;
	$(document).on('click', '.tab-buy', function(e){
		$('#buy').html(buy);
	});
	$(document).on('click', '.tab-payment', function(e){
		$('#payment').html(payment);
	});
	$(document).on('click', '.tab-delivery', function(e){
