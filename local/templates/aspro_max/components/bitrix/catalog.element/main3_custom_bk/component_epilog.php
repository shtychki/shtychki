								...
								<div class="tabs arrow_scroll">
									<ul class="nav nav-tabs font_upper_md">
										...
										<?foreach($arTabOrder as $value):?>
											...
											<?if($value == "buy"):?>
												<?if($bShowHowBuyTab):?>
													<li class="bordered rounded3 tab-buy <?=(!($iTab++) ? 'active' : '')?>"><a href="#buy" data-toggle="tab"><?=$arParams["TITLE_HOW_BUY"];?></a></li>
												<?endif;?>
											<?endif;?>
											<?//show payment block?>
											<?if($value == "payment"):?>
												<?if($bShowPaymentTab):?>
													<li class="bordered rounded3 tab-payment <?=(!($iTab++) ? 'active' : '')?>"><a href="#payment" data-toggle="tab"><?=$arParams["TITLE_PAYMENT"];?></a></li>
												<?endif;?>
											<?endif;?>
											<?//show delivery block?>
											<?if($value == "delivery"):?>
												<?if($bShowDeliveryTab):?>
													<li class="bordered rounded3 tab-delivery <?=(!($iTab++) ? 'active' : '')?>"><a href="#delivery" data-toggle="tab"><?=$arParams["TITLE_DELIVERY"];?></a></li>
												<?endif;?>
											<?endif;?>
											<?//show stores block?>
											<?if($value == "stores"):?>
												<?if($bShowStoresTab):?>
													<li class="bordered rounded3 tab-stores <?=(!($iTab++) ? 'active' : '')?>"><a href="#stores" data-toggle="tab"><?=$arParams["TAB_STOCK_NAME"];?></a></li>
												<?endif;?>
											<?endif;?>
											...
											<div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="desc">
												...
												<?if($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab):?>
													<div class="ordered-block">
														...
														<?//$APPLICATION->IncludeComponent('primepix:propertygroups', '.default',array('PROPERTIES' => $arResult['DISPLAY_PROPERTIES']),$component);?>
													</div>
												<?endif;?>
												<div class="ordered-block">
													<div class="text_block rounded3">
														<?$APPLICATION->IncludeFile(SITE_DIR."include/info_description.php", array(), array("MODE" => "html"));?>
													</div>
												</div>
												<?if($bShowDocs):?>
													<div class="ordered-block">
														...
														<?ob_start()?>
															<?$APPLICATION->IncludeComponent(
																...
																array(
																	...
																	"ELEMENT_CODE" => "",
//																	"ELEMENT_ID" => $arResult["ID"],
																	"ELEMENT_ID" => (isset($arResult['GENERAL_ID']) ? trim($arResult['GENERAL_ID']) : $arResult["ID"]),
																	"IBLOCK_ID" => $arParams["IBLOCK_ID"],
...
//											"ELEMENT_ID" => $arResult["ID"],
				...
				<?elseif($code == 'goods'):?>
					<?if($arParams['DETAIL_LINKED_GOODS_TABS'] != 'N'):?>
						<?//tabs mode?>
						<div class="ordered-block <?=$code?> 9999">
							<?$bNavTabs = false;?>
		...
		<div class="left_block sticky-sidebar-custom product-side">
			...
			<?if($arParams['USE_BIG_DATA'] == 'Y'):?>
				...
				<div class="bigdata-wrapper">
					<?//include_once($arParams['BIG_DATA_TEMPLATE']);?>
					<?include_once('page_blocks/element_bigdata_recommended.php');?>
				</div>
			<?endif;?>
...
<script>
	...
	$(document).on('click', '.tab-stores', function(e){
		//$('#stores').html(stores);
		setElementStore('<?=$templateData["STORES"]["OFFERS"];?>');
	});
</script>
