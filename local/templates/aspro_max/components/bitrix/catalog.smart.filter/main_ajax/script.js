...
JCSmartFilter.prototype.updateItem = function (PID, arItem, reset) {
  if (arItem.PROPERTY_TYPE === "N" || arItem.PRICE) {
	...
	let el_show = 5;
    let previous_label = '';
	let length = Object.keys(arItem.VALUES).length;

	// console.dir(arItem.VALUES);
	// console.dir(Object.keys(arItem.VALUES).length);
	// console.dir(Object.keys(arItem.VALUES).length);
      ...
      if (arItem.VALUES.hasOwnProperty(i)) {
        ...
        if (!!control) {
          ...
          if (value.DISABLED) {
            if (label) {
              disabled++;
              // BX.addClass(label, "disabled lst-order hidden");
              BX.addClass(label, "disabled filter-hidden");
              // if(count_search < 5 && length > j){
              // if(((el_show - count_search) < (length - j)) && (length - count_search - j) > (el_show - 1)){
              //   if(arItem.ID === '3894'){
              //     f++;
              //   }
              //   BX.removeClass(label, "--show");
              // } else if ($(label).not('--show') && (length - f - count_search) < el_show) {
              //   if(arItem.ID === '3894') {
              //     console.log(count_search); //1
              //     console.log(i); //xiaomi
              //     console.log(j); //5
              //     console.log(length); //7
              //     console.log(el_show); //5
              //   }
              //   BX.addClass(label, "--show");
              // }
				// BX.removeClass(label, "--show");
              // if(arItem.ID === '3894'){
              //   console.log(count_search); //0
              //   console.log(i); //apple
              //   console.log(j); //0
              //   console.log(length); //7
              //   console.log(el_show); //5
              ...
              if ((length - f - count_search) <= (el_show - count_search)) {
                BX.addClass(label, "--show");
                ...
                if((arItem.ID === '3894') || (arItem.ID === '4627') || (arItem.ID === '4616') || (arItem.ID === '3423')){
                  f++;
                }
                BX.removeClass(label, "--show");
                  if(arItem.ID === '3894') {
            ...
            if (label) {
              ...
              // }
              if ((disabled <= j && (j - disabled) < el_show && count_search < el_show ) || ($(label).hasClass('--show') && count_search < el_show)){
				BX.addClass(label, "--show");
			  } else {
				if ($(label).hasClass('--show') && value.CHECKED){
                  if(count_search >= el_show){
                    BX.removeClass(previous_label, "--show");
                  }
                    BX.addClass(label, "checked");
					// BX.removeClass(label, "--show");
				} else {
                  if($('.box_' + arItem.ID +' .show_all').hasClass('--active')){
                    BX.removeClass(label, "--show");
                    BX.addClass(label, "filter-hidden");
                  }
                }
			  }
              count_search++;
              previous_label = document.querySelector('[data-role="label_' + value.CONTROL_ID + '"]');
...
JCSmartFilter.prototype.filterCatalog = function (url, set_disabled, reset, count) {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      BX.onCustomEvent("onAjaxSuccessFilterMeta");
