									...
									<?if(!$arItem["IS_PROP_INLINE"]){?>
                                        ...
                                        <?if($arItem["FILTER_HINT"] && strpos( $arItem["FILTER_HINT"],'line')===false){?>
                                            <div class="filter_search">
                                                <input autocomplete="off" placeholder="<?=$arItem["NAME"]?>" data-id="<?=$arItem["ID"]?>" id="<?=$arItem["ID"]?>" class="search_inp" value="">
                                            </div>
                                        <?}?>
									<?}?>
									<?$style="";
									...
									<div class="bx_filter_block <?=(!$arItem["IS_PROP_INLINE"] && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=($arItem["IS_PROP_INLINE"] && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block scrollblock" : "");?>" <?=$style;?>>
										...
										switch ($arItem["DISPLAY_TYPE"]){
											...
											default://CHECKBOXES
												...
												<?foreach($arItem["VALUES"] as $val => $ar):?>
                                                <?
//                                                echo $j. ' - ' .$numVisiblePropValues.' - '.$isHidden.'<br>';
													if(!$ar["DISABLED"]):
                                                ?>
													<?if($j > $numVisiblePropValues && !$isHidden):
													//if($j > $numVisiblePropValues && !$isHidden && $k < $j):
														$isHidden = true;?>
<!--														<div class="hidden_values">-->
													<?endif;?>
													<?//if($ar["DISABLED"]):
													...
													<input
														...
														onclick="smartFilter.click(this)"
													/>
													<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?> <?=($arItem["FILTER_HINT"] && !$ar["CHECKED"] && $j > $numVisiblePropValues && !$isHidden ? 'off': '');?> <?= $isHidden?'hidden':'' ?>" for="<? echo $ar["CONTROL_ID"] ?>" data-name="<?=$ar["VALUE"];?>">
														<span class="bx_filter_input_checkbox">
															<span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
