...
//region Current section page url to component_epilog
$cp = $this->__component;
if (is_object($cp))
{
    $cp->arResult['CURRENT_SECTION_PAGE_CSTM'] = $arResult["SECTION_PAGE_URL"];
    $cp->SetResultCacheKeys(['CURRENT_SECTION_PAGE_CSTM']);
...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        if ($arItem['PROPERTIES']['ID']['VALUE'])
        {
            $arResult['ITEMS'][$key]['GENERAL_ID'] = $arItem['PROPERTIES']['ID']['VALUE'];

            $count_res = CIBlockElement::GetProperty($arItem['IBLOCK_ID'], $arResult['ITEMS'][$key]['PROPERTIES']['ID']['VALUE'], array(), array('CODE'=>'EXTENDED_REVIEWS_COUNT'));
            if($count = $count_res->Fetch()) {
                $arItem['PROPERTIES']['EXTENDED_REVIEWS_COUNT']['VALUE'] = IntVal($count["VALUE"]);
            }

            $raiting_res = CIBlockElement::GetProperty($arItem['IBLOCK_ID'], $arResult['ITEMS'][$key]['PROPERTIES']['ID']['VALUE'], array(), array('CODE'=>'EXTENDED_REVIEWS_RAITING'));
            if($raiting = $raiting_res->Fetch()) {
                $arItem['PROPERTIES']['EXTENDED_REVIEWS_RAITING']['VALUE'] = IntVal($raiting["VALUE"]);
            }
        }
