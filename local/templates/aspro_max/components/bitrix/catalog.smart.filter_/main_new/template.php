				...
				<div class="bx_filter_section bordered rounded3">
					<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
						<div class="bx_filter_parameters">
							...
							$numVisiblePropValues = \Bitrix\Main\Config\Option::get( "askaron.settings", "UF_CNT_FILTER_HIDE");
									...
									<div class="bx_filter_block  <?=($arItem["CODE"] != "IN_STOCK" && $arItem["DISPLAY_TYPE"] != 'A' ? '' : '');?> <?=($arItem["CODE"] != "IN_STOCK" && $arItem["PROPERTY_TYPE"]!="N" && ($arItem["DISPLAY_TYPE"] != "P" && $arItem["DISPLAY_TYPE"] != "R") ? "limited_block srollbar-custom scroll-deferred" : "");?>" <?=$style;?>>
										...
										switch ($arItem["DISPLAY_TYPE"]){
											...
											case "G"://CHECKBOXES_WITH_PICTURES
												...
												<?$valuesCount = 0;?>
												<?foreach ($arItem["VALUES"] as $val => $ar):?>
													<?if ($valuesCount >= $maxValuesCount) {break;}?>
													<?if($ar["VALUE"]){?>
													...
													<?$valuesCount++;?>
												<?endforeach?>
												<?if(count($arItem["VALUES"]) > $maxValuesCount):?>
											...
											default://CHECKBOXES
												...
												<?$class = '';?>
												<?foreach($arItem["VALUES"] as $val => $ar):?>
													<?if ($valuesCount >= $maxValuesCount) {$class = 'filter-hide-on-desktop';}?>
													<?if($j > $numVisiblePropValues && !$isHidden):
														$isHidden = true;?>
														<div class="hidden_values">
													<?endif;?>
													<input
														...
														class="<?=$class;?>"
													/>
													<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=$class;?> <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
														<span class="bx_filter_input_checkbox">
												...
												<?endforeach;?>
												<?if(count($arItem["VALUES"]) > $maxValuesCount):?>
													<span style="position:relative;" class="filter-hide-on-mobile">
														<span class="filter-show-more-btn">Показать ещё... (<?=count($arItem["VALUES"])?> шт.)</span>
														<?$columnsCount = (ceil(count($arItem["VALUES"])/5) < 5)? ceil(count($arItem["VALUES"])/5): 5;?>
														<div class="filter-more-hidden-block" style="width:<?=$columnsCount * 170;?>px; grid-template-columns: repeat(<?=$columnsCount;?>, 1fr);">
															<?foreach($arItem["VALUES"] as $val => $ar):?>
																<?if($j > $numVisiblePropValues && !$isHidden):
																	$isHidden = true;?>
																	<div class="hidden_values">
																...
																<input
																	...
																	value="<? echo $ar["HTML_VALUE"] ?>"
																	name="<? echo $ar["CONTROL_NAME"] ?>"
																	id="<? echo $ar["CONTROL_ID"] ?>_full"
																	<? echo $ar["DISABLED"] ? 'disabled class="disabled"': '' ?>
																	<? echo $ar["CHECKED"]? 'checked="checked"': '' ?>
																	onclick="smartFilter.click(this)"
																/>
																<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>_full">
																	<span class="bx_filter_input_checkbox">
