...
<?if($arResult['FORM_TYPE'] === 'login'):?>
	...
	<?else:?>
        <?if(
            $arResult['ERROR'] &&
            $arResult['ERROR_MESSAGE']['TYPE'] === 'ERROR' &&
            $arResult['ERROR_MESSAGE']['ERROR_TYPE'] === 'LOGIN'
        ):
            $arResult['ERROR_MESSAGE']['MESSAGE'] = GetMessage('AUTH_ERROR_PHONE');
        endif;?>
		<div id="ajax_auth" class="auth-page pk-page">
			<div class="auth_wrapp">
				<div class="wrap_md1">
					<div class="form custom_login">
						<form id="auth-page-form" name="system_auth_form<?=$arResult['RND']?>" method="post" target="_top" action="<?=$arParams['AUTH_URL']?>?login=yes">
							...
							<div class="form_body">
								...
								<?else:?>
									<div class="form-control phone_or_login">
                                        <input type="hidden" name="AUTH_PHONE" value="1" />
                                        <input type="hidden" name="USER_LOGIN" id="USER_LOGIN_PHONE" value="<?=$arResult['USER_LOGIN']?>"/>
										<label for="USER_LOGIN_POPUP"><span><?=GetMessage('auth_phone_number')?>&nbsp;<span class="required-star">*</span></span></label>
										<input type="text" name="USER_LOGIN_FIELD" id="USER_LOGIN_POPUP" class="required phone_input" maxlength="50" value="<?=substr($arResult['USER_LOGIN'],2)?>" autocomplete="on" tabindex="1"/>
									</div>
									<div class="form-control password_blk">
...
<!--                                    <a class="forgot" href="--><?//=$arResult['AUTH_FORGOT_PASSWORD_URL']?><!--" tabindex="4">--><?//=GetMessage('AUTH_FORGOT_PASSWORD_2')?><!--</a>-->
				...
				$('#auth-page-form').validate({
					...
					submitHandler: function(form){
						...
						if($form.valid()){
                            ...
                            dataLayer.push({'event': 'en_cabinet'});
			...
			$('#auth-page-form .phone_or_login input').phoneOrLogin(function(input, test){
                ...
                var login = $form.find('input[name=AUTH_PHONE]').val();
                if(login == false) {
                    ...
                    if(test.bPossiblePhone){
                        ...
                        $form.find('input[name=USER_PHONE_NUMBER]').val(test.value);
                        var new_login = test.value.replace(/[-() +]/g,'');
                        $form.find('input[name=USER_LOGIN]').val(new_login);
                    }
                }
			});
            if(arMaxOptions['THEME']['PHONE_MASK'].length){
                var base_mask = arMaxOptions['THEME']['PHONE_MASK'].replace( /(\d)/g, '_' );
                $('#auth-page-form input.phone_input').inputmask('mask', {'mask': arMaxOptions['THEME']['PHONE_MASK'] });
                $('#auth-page-form input.phone_input').blur(function(){
