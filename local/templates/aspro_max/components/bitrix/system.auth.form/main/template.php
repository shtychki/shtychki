...
lib\PhoneAuthMod::modifyResult($arResult, $arParams);
$authType = 'login';
						...
						<form id="auth-page-form" name="system_auth_form<?=$arResult['RND']?>" method="post" target="_top" action="<?=$arParams['AUTH_URL']?>?login=yes">
							...
							<?if($bUsePhoneInput):?>
									<?if($arResult['SHOW_SMS_FIELD']):?>
                                        ...
                                        <?if($arResult['PROFILE_URL']):?>
										<input type="hidden" name="PROFILE_URL" value="<?=htmlspecialcharsbx($arResult["PROFILE_URL"])?>" />
                                        <?endif;?>
										<div class="form-control">
							...
							<div class="form_footer">
                                <div class="text-sms">
                                    Необходимо подтвердить номер телефона через СМС-код, который будет выслан в viber либо по смс
                                </div>
								...
								<div class="buttons clearfix">
									<?if($arResult['PHONE_AUTH_PARAMS']['USE']):?>
										...
										<?else:?>
											...
											<!--noindex-->
                                            <a href="<?=$arResult['AUTH_REGISTER_URL'];?>" rel="nofollow" class="btn btn-transparent-border-color btn-lg pull-right register hidden" tabindex="6"><?=GetMessage('AUTH_REGISTER_NEW')?></a>
                                            <?if($arResult['PHONE_AUTH_PARAMS']['USE'] && !$arResult['SHOW_SMS_FIELD']):?>
                                                <div class="licence_block"><label><?$APPLICATION->IncludeFile(SITE_DIR."include/auth_phone_licenses_text.php", Array(), Array("MODE" => "html", "NAME" => "LICENSES"));?></label></div>
                                            <?endif;?>
                                            <!--/noindex-->
										<?endif;?>
									<?else:?>
										...
										<!--noindex-->
                                        <a href="<?=$arResult['AUTH_REGISTER_URL'];?>" rel="nofollow" class="btn btn-transparent-border-color btn-lg pull-right register" tabindex="6"><?=GetMessage('AUTH_REGISTER_NEW')?></a>
                                        <!--/noindex-->
									<?endif;?>
										...
										if(!$button.hasClass('loadings')){
											...
											$.ajax({
												...
												if($(html).find('.alert').length){
													...
													}else if($form.find('input[name=PROFILE_URL]').length){
