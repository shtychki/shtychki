...
if (!$typeTmpSKU || $sectionTizers || !$sectionHelpText || !$linkedArticles) {
	if($arSection["DEPTH_LEVEL"] > 2){
		$arSectionParent = CMaxCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMaxCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "UF_OFFERS_TYPE", "UF_SECTION_TIZERS", 'UF_LINKED_BLOG', "UF_HELP_TEXT", "UF_ELEMENT_PROP_LIST"));
