...
/*hover animate*/
	...
	if (!$(e.target).closest('.menu-row .mega-menu table').length) {
			if($(e.target).siblings().is(':not(.colored_theme_hover_text.more_items.with_dropdown)')) {

				var _this = $('.menu-row .mega-menu table td'),
	...
	$(document).on('click', '.menu .mega-menu table td, .menu-row .mega-menu table td', function(e){
		e.preventDefault();
		
		var _this = $(this),
			menu = _this.find('> .wrap > .dropdown-menu');
		
		if(!_this.hasClass('open-menu'))
		{
			//menu = _this.find('> .wrap > .dropdown-menu');
